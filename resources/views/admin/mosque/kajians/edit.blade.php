<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl">Edit Kajian</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 max-w-5xl mx-auto">
        <div class="bg-white dark:bg-gray-800 border rounded-xl shadow-sm">
            <div class="px-6 py-4 border-b">
                ...
                <div class="text-sm text-slate-500">Ubah data kajian.</div>
            </div>
            <form method="POST" action="{{ route('kajians.update', $item) }}" class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="speaker" class="mt-2 w-full" value="{{ old('speaker', $item->speaker) }}">
                </div>
                <div>
                    <label class="block text-sm font-medium">Mulai</label>
                    <input type="datetime-local" name="starts_at" class="mt-2 w-full" value="{{ old('starts_at', optional($item->starts_at)?->format('Y-m-d\TH:i')) }}" required>
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="location" class="mt-2 w-full" value="{{ old('location', $item->location) }}">
                </div>
                <div class="md:col-span-2">
