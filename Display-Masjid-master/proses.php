...
{
    protected $file = 'db/database.json';
    ...
    public function __construct($id)
    {
        ...
        if ($id == 'login' || $id == 'logout') {
            ...
            $this->$id();
        } elseif ($id == 'changeDbCheck') {
            $this->getDatabase();
            $this->$id();
        } elseif ($this->verification($id)) {
            $this->getDatabase();
            $this->$id();
        }
    }
    ...
    private function login()
    {
        ...
        } elseif ($user != $db['akses']['user'] || $pass != $db['akses']['pass']) {
    ...
    private function formSave()
    {
        ...
        if ($id == 'info') {
            $dt = [$dt['r1'], $dt['r2'], $dt['r3'], $dt['active']];
        } elseif ($id == 'running_text') {
            $dt = $dt['text'];
        } elseif ($id == 'prayTimesAdjust') {
            ...
            } elseif ($dt['password_baru'] != $dt['ulangi_password_baru']) {
                $this->retError('Password baru tidak sama...');
            } elseif (strlen($dt['password_baru']) < 8) {
    ...
    private function getWallpaper()
    {
        ...
        $files = array_diff(scandir($dir), ['.', '..', 'Thumbs.db']);
